...
`SubagentStop` hook script, example hook registration, and setup notes for
ntfy, Pushover, or Apprise. The script reads Codex hook JSON from stdin,
derives project context from the session `cwd`, and keeps notification tokens
and private topics in environment variables instead of repo files.
...
entire global `~/.codex/hooks.json`.
