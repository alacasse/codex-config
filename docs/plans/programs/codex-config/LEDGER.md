...
- Create a dispatch or runway only when one row is explicitly chosen by
  `plan-batch`. CCFG-26A is completed and reconciled. The issue #66 slice-shape
  policy correction is the only queued batch; selected dispatch and active
  runway are `None`. It must close before a later explicit stable `plan-batch`
  invocation may select CCFG-26B.
...
| CCFG-26. Transfer Execution and Closeout Ownership to `work-batch` | Pending | [COR-009](https://github.com/alacasse/codex-config/blob/caf343a14bf8dae5ba3bfda6d8ab974929bb4c7c/docs/design/command-owner-redesign/07-implementation-ledger-intake.md#cor-009--ccfg-26--transfer-execution-and-closeout-ownership-to-work-batch), GitHub issue #66, `findings/slice-shape-policy-direction.md`, `findings/command-owner-redesign-planning-execution-carry-forward.md`, and `findings/github-issue-62-stable-runway-dogfooding-bootstrap.md` | Command-owner redesign / execution ownership | Execute only `batches/ccfg-26-slice-shape-policy-correction/runway.md` through a later explicit `work-batch` | CCFG-26A remains closed historical evidence at candidate commit `a0835f1`. The queued issue #66 corrective preparation decouples slice shape from migration risk, resolves one project-owned vertical-default policy with justified horizontal override, persists selected shape, and preserves migration-specific evidence without historical compatibility. Its exact independent planning review is clean. CCFG-26B through CCFG-26E retain later execution-ownership work and remain unselected until this correction closes. |
...
Active batch: `ccfg-26-slice-shape-policy-correction`, status `queued`.
Selected dispatch and active runway are `None`; the queued batch is
`batches/ccfg-26-slice-shape-policy-correction/runway.md`.
...
| `ccfg-26-slice-shape-policy-correction` | queued | `docs/plans/programs/codex-config/batches/ccfg-26-slice-shape-policy-correction/dispatch.md` | `docs/plans/programs/codex-config/batches/ccfg-26-slice-shape-policy-correction/runway.md` | CCFG-26 | One vertical contract-narrowing preparation slice before CCFG-26B. Exact planning review is clean at dispatch SHA-256 `e52002ca` and runway SHA-256 `23efb8df`. Preserve CCFG-26A as historical evidence, consume one project-owned shape policy, keep migration evidence independent, and add no historical compatibility. |
| `ccfg-26b-fresh-slice-flight` | candidate | None | None | CCFG-26 | Deferred. Queue-to-worker/validation/review/commit through a fresh coordinator and durable flight receipt. Requires both CCFG-26A and the queued slice-shape correction to close, then a later explicit stable `plan-batch`; candidate planning is not authoritative before CCFG-29. |
...
3. CCFG-25, CCFG-26A, and CCFG-34 are closed; parent CCFG-26 is `Pending`.
4. Execute only
   `batches/ccfg-26-slice-shape-policy-correction/runway.md`; its one vertical
   contract-narrowing slice has a clean exact planning review.
5. CCFG-26B remains unselected and blocked on this corrective preparation
   closeout. A later explicit stable `plan-batch` invocation owns its selection.
   Stable `plan-batch` plus the temporary CCFG-34 policy remain the canonical
   planning path for CCFG-26 through CCFG-29.
6. Keep CCFG-26B through CCFG-26E, CCFG-27 through CCFG-29, and all older open
   rows unselected until an explicit future command chooses one.
