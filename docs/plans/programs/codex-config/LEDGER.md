...
| CCFG-24. Transfer Intake Ownership to `add-to-ledger` | Pending | [COR-007](https://github.com/alacasse/codex-config/blob/caf343a14bf8dae5ba3bfda6d8ab974929bb4c7c/docs/design/command-owner-redesign/07-implementation-ledger-intake.md#cor-007--ccfg-24--transfer-intake-ownership-to-add-to-ledger), `findings/ccfg-24-two-batch-execution-amendment.md`, and `findings/ccfg-24a-add-to-ledger-v1-decision-amendment.md` | Command-owner redesign / intake ownership | Execute the current two-slice CCFG-24A runway only on a later explicit `work-batch` request | CCFG-24 remains one COR-007 finding split into preparation and cutover. The failed CCFG-24A attempt is preserved as historical evidence; its blocker is resolved by clarifying that the command owner reevaluates upstream semantics and derives the exact prepared-store key internally while DEC-037 and the apply-only store remain unchanged. Current CCFG-24A is queued and executable. CCFG-24B and CCFG-25 remain unselected and unplanned. |
...
Queued batch (executable on a later explicit `work-batch` request):
...
| `ccfg-24a-intake-owner-preparation` | queued | `docs/plans/programs/codex-config/batches/ccfg-24a-intake-owner-preparation/dispatch.md` | `docs/plans/programs/codex-config/batches/ccfg-24a-intake-owner-preparation/runway.md` | CCFG-24 | Current two-slice runway consumes the accepted decision amendment, implements and candidate-installs the bounded owner over the unchanged apply-only store, then binds scenarios and measures preparation evidence. The failed three-slice attempt remains at `execution-report.md`, `blocked-dispatch.md`, and `blocked-runway.md`; it must not be resumed. No cleanup, CCFG-24B planning, or CCFG-25 work is authorized. |

## Recommended Work Order

1. Start pickup from root and program `CURRENT.md`.
2. CCFG-18 through CCFG-23 and CCFG-30 through CCFG-33 are closed.
3. CCFG-24A is the only queued artifact and is executable only through a later
   explicit `work-batch` request. Execute `runway.md`; never resume
   `blocked-runway.md` or recreate its decision slice.
4. Keep CCFG-24 `Pending`. Do not create any CCFG-24B dispatch or runway until
   CCFG-24A executes and closes with the required `Prepared` evidence.
