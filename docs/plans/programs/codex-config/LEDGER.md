...
| CCFG-25. Transfer Planning Ownership to `plan-batch` | Pending | [COR-008](https://github.com/alacasse/codex-config/blob/caf343a14bf8dae5ba3bfda6d8ab974929bb4c7c/docs/design/command-owner-redesign/07-implementation-ledger-intake.md#cor-008--ccfg-25--transfer-planning-ownership-to-plan-batch), `findings/ccfg-25-planning-quality-amendment.md`, and `findings/command-owner-redesign-planning-execution-carry-forward.md` | Command-owner redesign / planning ownership | Resolve Slice 3 changed-file BasedPyright scope | Slice 3 static convergence commit `89671eceb9103039e7e6660e73837827c167a3a1` is focused-green and reviewed. The validation-only amendment is not executable: its explicit all-changed-Python-files command exits `123` with 120 errors and 3 warnings because it analyzes tests and a fixture outside the configured `scripts` scope. Fresh planning review returned findings. Installation, exact acceptance, final review, and closeout have not run; CCFG-26 remains preserved and no successor is selected. |
...
Active batch: `ccfg-25-planning-ownership-transfer`.
...
| `ccfg-25-planning-ownership-transfer` | active | `docs/plans/programs/codex-config/batches/ccfg-25-planning-ownership-transfer/dispatch.md` | `docs/plans/programs/codex-config/batches/ccfg-25-planning-ownership-transfer/runway.md` | CCFG-25 | Slices 1 and 2 are complete; Slice 3 static convergence is committed as `89671eceb9103039e7e6660e73837827c167a3a1`. Its exact validation-only amendment has independent review findings because the all-changed-Python-files gate expands beyond configured BasedPyright scope. No installation, closeout, or successor selection. |

## Recommended Work Order

1. Start pickup from root and program `CURRENT.md`.
2. CCFG-18 through CCFG-24 and CCFG-30 through CCFG-33 are closed.
3. Keep the same `ccfg-25-planning-ownership-transfer` active and await a further
   validation-only amendment resolving whether the changed-file BasedPyright gate
   covers all changed Python files or only the configured project scope. Do not
   edit configuration or chase diagnostics, create closeout, or select a successor.
...
- CCFG-25 is active at
  ...
  Slice 1 is committed as `5aa5add1251d1e4b3630a9678fdec244949cf691`, and
  every later handoff requires a freshly prepared strict lease.
