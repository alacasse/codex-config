...
- Create a dispatch or runway only when one row is explicitly chosen by
  ...
  first bounded execution-state foundation is independently reviewed and
  queued. No implementation Slice is active and no successor is selected.
...
- Clean CCFG-26 execution-state foundation planning review:
...
| `ccfg-26-execution-state-foundation` | queued | `docs/plans/programs/codex-config/batches/ccfg-26-execution-state-foundation/dispatch.md` | `docs/plans/programs/codex-config/batches/ccfg-26-execution-state-foundation/runway.md` | CCFG-26 | First bounded execution foundation; independent planning review is clean. Two vertical Slices are pending, automatic continuation is deferred, no implementation has started, and no successor is selected. |
...
4. On a later explicit stable `work-batch`, consume only the queued
   `ccfg-26-execution-state-foundation` runway. Refresh strict identities and a
   ready live lease, preserve the exact caller-supplied runtime root, apply the
   temporary CCFG-34 policy, and advance exactly the next incomplete Slice
   through implementation, validation, review, commit, receipt, and archive.
   Stop before another Slice. Automatic successful continuation remains later
   required CCFG-26 work and is outside this queued batch.
