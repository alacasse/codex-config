...
| CCFG-26. Transfer Execution and Closeout Ownership to `work-batch` | Pending | [COR-009](https://github.com/alacasse/codex-config/blob/caf343a14bf8dae5ba3bfda6d8ab974929bb4c7c/docs/design/command-owner-redesign/07-implementation-ledger-intake.md#cor-009--ccfg-26--transfer-execution-and-closeout-ownership-to-work-batch), `findings/ccfg-26-execution-state-authority-direction.md`, `notes/ccfg-26-replan-analysis-and-chatgpt-pro-handoff.md`, `notes/ccfg-26-execution-state-design-contract.md`, `notes/ccfg-26-execution-state-design-review.md`, `docs/adr/0003-canonical-batch-execution-state.md`, GitHub issue #66, `findings/slice-shape-policy-direction.md`, `findings/command-owner-redesign-planning-execution-carry-forward.md`, and `findings/github-issue-62-stable-runway-dogfooding-bootstrap.md` | Command-owner redesign / execution ownership | On a later explicit stable `work-batch`, consume the queued runway together with `amendment.md` and `amendment-review.md`, execute exactly real Slice 1 with stable support, then stop before another Slice | CCFG-26 and COR-009 remain open. The first bounded execution foundation is queued with two vertical Slices and a clean bootstrap correction. Stable `work-batch` controls both real implementation Slices, one invocation at a time. The candidate runner/public-`work-batch` seam is proven only against disposable fixtures, with no real Batch Execution State or durable temporary root. The original review is historical only. Automatic successful continuation, recovery, finalization, target closeout, and displaced-owner narrowing remain required later CCFG-26 work. CCFG-26B remains superseded evidence only; CCFG-26C through CCFG-26E remain unselected conceptual concerns. No implementation has started and no successor is selected. |
...
| `ccfg-26-execution-state-foundation` | queued | `docs/plans/programs/codex-config/batches/ccfg-26-execution-state-foundation/dispatch.md` | `docs/plans/programs/codex-config/batches/ccfg-26-execution-state-foundation/runway.md` | CCFG-26 | First bounded execution foundation, corrected by `amendment.md` and its clean exact review. Stable `work-batch` controls both real Slices; candidate execution is fixture-only, with no real Batch Execution State or durable temporary root. The original review is historical only. Automatic continuation is deferred, no implementation has started, and no successor is selected. |
...
4. On a later explicit stable `work-batch`, consume only the queued
   `ccfg-26-execution-state-foundation` runway together with its authoritative
   `amendment.md` and clean `amendment-review.md`. Refresh strict identities and
   a ready live lease, apply the temporary CCFG-34 policy, execute exactly real
   Slice 1 with the stable controller, and stop after implementation,
   validation, review, commit, receipt, and archive. A later explicit stable
   invocation controls real Slice 2; its candidate execution proof is
   fixture-only and creates no real Batch Execution State or durable temporary
   root. Automatic successful continuation remains later required CCFG-26 work
   and is outside this queued batch.
