...
- Create a dispatch or runway only when one row is explicitly chosen by
  ...
  explicit user direction. The queue is empty, no batch is selected or active,
  CCFG-26's formal design gate is satisfied and the row is `Ready` for a later
  explicit `plan-batch`, and no successor is selected.
...
| CCFG-26. Transfer Execution and Closeout Ownership to `work-batch` | Ready | [COR-009](https://github.com/alacasse/codex-config/blob/caf343a14bf8dae5ba3bfda6d8ab974929bb4c7c/docs/design/command-owner-redesign/07-implementation-ledger-intake.md#cor-009--ccfg-26--transfer-execution-and-closeout-ownership-to-work-batch), `findings/ccfg-26-execution-state-authority-direction.md`, `notes/ccfg-26-replan-analysis-and-chatgpt-pro-handoff.md`, `notes/ccfg-26-execution-state-design-contract.md`, `notes/ccfg-26-execution-state-design-review.md`, `docs/adr/0003-canonical-batch-execution-state.md`, GitHub issue #66, `findings/slice-shape-policy-direction.md`, `findings/command-owner-redesign-planning-execution-carry-forward.md`, and `findings/github-issue-62-stable-runway-dogfooding-bootstrap.md` | Command-owner redesign / execution ownership | On a later explicit `plan-batch`, select the first bounded execution-foundation batch. It must end in one independently useful state through the real candidate runner/`work-batch` seam. Automatic continuation remains a CCFG-26 completion requirement and belongs to this first batch only if planning proves the combined semantic, validation, and rollback boundary proportionate. Resolve strict identities/runtime root and stop before implementation | CCFG-26 and COR-009 remain open. The user-amended formal design gate is independently reviewed clean. One fresh coordinator still advances at most one Slice, but automatic successful same-batch continuation is required before normal use or CCFG-26 closeout. CCFG-26A and the slice-shape correction remain closed historical evidence. CCFG-26B was superseded before implementation; its nine planning artifacts remain evidence only and must not be executed, resumed, or amended. The queue is empty and no successor is selected. CCFG-26C through CCFG-26E remain unselected conceptual concerns whose boundaries and dependencies must be re-evaluated after the execution foundation. |
...
3. CCFG-25, CCFG-26A, CCFG-34, and the issue #66 slice-shape correction are
   closed; parent CCFG-26 is `Ready`, the queue is empty, and CCFG-26B is
   superseded historical evidence.
4. Only a later explicit stable `plan-batch` request may select exactly one
   bounded CCFG-26 execution-foundation batch from the accepted execution-state
   design. It must derive slice count from semantic interface/behavior/test/
   rollback boundaries and end in one independently useful state through the
   real candidate runner/`work-batch` seam. Automatic successful continuation
   remains required for CCFG-26 completion but enters this first batch only when
   planning proves the combined boundary proportionate. The planner must refresh
   strict stable/candidate identities, resolve the exact runtime root from
   authorized policy or explicit execution input, apply the temporary CCFG-34
   policy, queue one batch, and stop before implementation.
...
- The later CCFG-26B dispatch, runway, amendment, and correction bundle was
  ...
  evidence boundary. The parent CCFG-26 remains open and is now `Ready` because
  its formal design gate is accepted and reviewed clean; the queue is empty and
  no successor was selected.
