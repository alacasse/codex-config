...
| CCFG-25. Transfer Planning Ownership to `plan-batch` | Pending | [COR-008](https://github.com/alacasse/codex-config/blob/caf343a14bf8dae5ba3bfda6d8ab974929bb4c7c/docs/design/command-owner-redesign/07-implementation-ledger-intake.md#cor-008--ccfg-25--transfer-planning-ownership-to-plan-batch), `findings/ccfg-25-planning-quality-amendment.md`, and `findings/command-owner-redesign-planning-execution-carry-forward.md` | Command-owner redesign / planning ownership | Await user direction on the outside-ceiling runner safety owner | Slice 2 stopped at independent implementation review. `scripts/architecture_program_runner_change_allowance.py` rejects complete `plan-batch` transaction artifacts before compatibility `create-spec`; it is outside the exact amended ceiling. Candidate diff remains uncommitted. Preserve CCFG-26 and select no successor. |
