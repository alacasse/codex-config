...
- Create a dispatch or runway only when one row is explicitly chosen by
  ...
  a clean bounded two-slice amendment; no batch is active and no successor is
  selected.
...
| CCFG-26. Transfer Execution and Closeout Ownership to `work-batch` | Pending | [COR-009](https://github.com/alacasse/codex-config/blob/caf343a14bf8dae5ba3bfda6d8ab974929bb4c7c/docs/design/command-owner-redesign/07-implementation-ledger-intake.md#cor-009--ccfg-26--transfer-execution-and-closeout-ownership-to-work-batch), GitHub issue #66, `findings/slice-shape-policy-direction.md`, `findings/command-owner-redesign-planning-execution-carry-forward.md`, and `findings/github-issue-62-stable-runway-dogfooding-bootstrap.md` | Command-owner redesign / execution ownership | Execute queued CCFG-26B only through its clean bounded amendment and later explicit stable `work-batch` invocations; execute one amended implementation slice per invocation under the temporary policy | CCFG-26A remains closed historical evidence at candidate commit `a0835f1`. The original issue #66 closeout remains historical at candidate commit `8a93319`; `batches/ccfg-26-slice-shape-policy-correction/post-closeout-correction.md` records the substitutive semantic-authority correction at candidate commit `5c5ec9d`. CCFG-26B remains the same queued runway; `batches/ccfg-26b-fresh-slice-flight/amendment.md` and `amendment-review.md` bind its clean two-slice execution basis. CCFG-26C through CCFG-26E remain unselected. |
...
4. Execute only the queued CCFG-26B runway plus its exact clean amendment basis
   through later explicit stable `work-batch` invocations, one amended
   implementation slice per invocation; no batch is active and implementation
   has not started.
