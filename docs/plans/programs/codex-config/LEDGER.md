...
- CCFG-33 may close only if COR-006 behavior remains green, reporter-owned
  nested pytest is removed, fast and acceptance gates are distinct, obsolete
  topology/migration-retention tests are deleted or migrated rather than used
  to restore old code, and before/after cost evidence is recorded. Its closeout
