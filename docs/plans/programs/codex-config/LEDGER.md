...
| CCFG-24. Transfer Intake Ownership to `add-to-ledger` | Open | [COR-007](https://github.com/alacasse/codex-config/blob/caf343a14bf8dae5ba3bfda6d8ab974929bb4c7c/docs/design/command-owner-redesign/07-implementation-ledger-intake.md#cor-007--ccfg-24--transfer-intake-ownership-to-add-to-ledger) plus deletion-first carry-forward in `findings/command-owner-redesign-planning-execution-carry-forward.md` | Command-owner redesign / intake ownership | Wait for CCFG-33 | Transfer intake and ledger-mutation semantics; remove APR intake, the legacy-removal escape hatch, and replaced CCFG-23 intake adapters/tests in the same work. Unselected. |
| CCFG-25. Transfer Planning Ownership to `plan-batch` | Open | [COR-008](https://github.com/alacasse/codex-config/blob/caf343a14bf8dae5ba3bfda6d8ab974929bb4c7c/docs/design/command-owner-redesign/07-implementation-ledger-intake.md#cor-008--ccfg-25--transfer-planning-ownership-to-plan-batch), `findings/ccfg-25-planning-quality-amendment.md`, and `findings/command-owner-redesign-planning-execution-carry-forward.md` | Command-owner redesign / planning ownership | Wait for CCFG-24; consume completed CCFG-21 planning contracts and transaction owner | Implement #51, #52, and #54 through the only queue mutator, then delete replaced fixture planning logic and APR/Batch Runway ownership tests in the same work. Unselected. |
| CCFG-26. Transfer Execution and Closeout Ownership to `work-batch` | Open | [COR-009](https://github.com/alacasse/codex-config/blob/caf343a14bf8dae5ba3bfda6d8ab974929bb4c7c/docs/design/command-owner-redesign/07-implementation-ledger-intake.md#cor-009--ccfg-26--transfer-execution-and-closeout-ownership-to-work-batch) plus live execution boundary in `findings/command-owner-redesign-planning-execution-carry-forward.md` | Command-owner redesign / execution ownership | Wait for CCFG-25 | Make Planning State the sole semantic currentness gate; transfer all execution/closeout decisions; remove APR/Batch Runway ownership and duplicate CCFG-23 worker/validator/reviewer/committer fixtures/tests in the same work. Unselected. |
| CCFG-27. Prepare and Rehearse Candidate Cutover | Open | [COR-010](https://github.com/alacasse/codex-config/blob/caf343a14bf8dae5ba3bfda6d8ab974929bb4c7c/docs/design/command-owner-redesign/07-implementation-ledger-intake.md#cor-010--ccfg-27--prepare-and-rehearse-candidate-cutover) plus live bridge-readiness gate in `findings/command-owner-redesign-planning-execution-carry-forward.md` | Command-owner redesign / cutover preparation | Wait for CCFG-26 | Prove candidate-installed bridge parity and replace synthetic readiness fixtures where real candidate behavior becomes authoritative; retain only named fault boundaries. Master remains canonical planning authority. Unselected. |
| CCFG-28. Remove Legacy Owners and Commit Final Cutover | Open | [COR-011](https://github.com/alacasse/codex-config/blob/caf343a14bf8dae5ba3bfda6d8ab974929bb4c7c/docs/design/command-owner-redesign/07-implementation-ledger-intake.md#cor-011--ccfg-28--remove-legacy-owners-and-commit-final-cutover) plus live switch gate in `findings/command-owner-redesign-planning-execution-carry-forward.md` | Command-owner redesign / deletion and final switch | Wait for CCFG-27 bridge parity and rehearsal proof | Delete legacy owners, prove the clean candidate switch, and delete CCFG-23 cutover fixtures superseded by real behavior or reduce them to named fault boundaries. Unselected; distinct from CCFG-11. |
| CCFG-29. Contract-First Convergence and Final Integration | Open | [COR-012](https://github.com/alacasse/codex-config/blob/caf343a14bf8dae5ba3bfda6d8ab974929bb4c7c/docs/design/command-owner-redesign/07-implementation-ledger-intake.md#cor-012--ccfg-29--contract-first-convergence-and-final-integration) plus live convergence obligations in `findings/command-owner-redesign-planning-execution-carry-forward.md` | Command-owner redesign / convergence and integration | Wait for CCFG-28 | Merge candidate into latest authoritative master, preserve target behavior, delete the complete temporary bridge and every remaining CCFG-23 migration-retention adapter/model/wrapper/test, then rebind default Codex home to master. Unselected. |
...
| CCFG-31. Narrow ready/blocked live-lease preflight | Closed | GitHub issue #53; `batches/ccfg-31-narrow-live-lease-preflight/closeout.md` | Batch Runway / cross-checkout execution lifecycle | None | Closed by `batches/ccfg-31-narrow-live-lease-preflight/closeout.md`: exact queue-transaction proof, ready/blocked behavior, preserved strict per-handoff safety, broad-protocol deletion, final validation, and clean exact-range review are complete. CCFG-29 remains the sole bridge-removal owner; no successor was selected. |
...
| CCFG-33. Simplify CCFG-23 acceptance execution and delete test-retained topology | Open | GitHub issue #56; `findings/github-issue-56-ccfg-23-harness-simplification.md`; CCFG-23 execution retrospective | Command-owner redesign / behavioral-harness execution | Explicit `plan-batch` should select this before CCFG-24 | Replace recursive reporter-owned pytest with one exact-commit acceptance receipt, split fast/acceptance gates, delete obsolete topology/migration-retention tests with the mechanism, and record a net-negative implementation plus old/new performance evidence. CCFG-23 remains closed. |
...
4. The next recommended command-owner-redesign selection is CCFG-33. It must
   close before CCFG-24.
5. Keep CCFG-24 through CCFG-29 and all older open rows unselected until a
   later explicit `plan-batch` request.
...
- Close CCFG-18 through CCFG-29 and CCFG-33 only from the acceptance evidence
  and stop boundaries in the local intake files, live amendments, and immutable
...
- CCFG-33 may close only with the same COR-006 behavior, a net-negative harness
  and test diff, one exact-commit acceptance receipt path, pure report
  rendering, deletion or migration of topology-preserving tests, and measured
  old/new duration and subprocess evidence. Its closeout selects no successor.
...
- CCFG-24 through CCFG-29 must delete CCFG-23 migration-retention adapters,
  duplicate workflow models, and preserving tests as the corresponding real
  owner becomes authoritative. A temporarily retained surface must name its
...
- CCFG-29 merges candidate into latest authoritative `master`, removes the
  ...
  removes every remaining test-retained CCFG-23 compatibility surface, and
  restores integrated `master` as the default Codex-home toolchain source.
