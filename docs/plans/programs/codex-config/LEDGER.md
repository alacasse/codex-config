...
| CCFG-25. Transfer Planning Ownership to `plan-batch` | Pending | [COR-008](https://github.com/alacasse/codex-config/blob/caf343a14bf8dae5ba3bfda6d8ab974929bb4c7c/docs/design/command-owner-redesign/07-implementation-ledger-intake.md#cor-008--ccfg-25--transfer-planning-ownership-to-plan-batch), `findings/ccfg-25-planning-quality-amendment.md`, and `findings/command-owner-redesign-planning-execution-carry-forward.md` | Command-owner redesign / planning ownership | Await command-only required-green amendment authority | The Change Allowance correction is independently verified, but the exact two-skill validation command activates catalog relationship checks and exits `1` because external mechanisms are not CLI-configurable. Preserve exact candidate diff `815c4ad7b15e9143cb95e3f5790440021416ccb28bd8120731ac92314c8b023e` uncommitted; preserve CCFG-26 and select no successor. |
...
| `ccfg-25-planning-ownership-transfer` | active | `docs/plans/programs/codex-config/batches/ccfg-25-planning-ownership-transfer/dispatch.md` | `docs/plans/programs/codex-config/batches/ccfg-25-planning-ownership-transfer/runway.md` | CCFG-25 | Slice 1 committed as `5aa5add1251d1e4b3630a9678fdec244949cf691`; second-amendment runner correction is verified; Slice 2 is blocked only by the exact combined skill-contract command; candidate diff uncommitted; no successor selection. |

## Recommended Work Order

1. Start pickup from root and program `CURRENT.md`.
2. CCFG-18 through CCFG-24 and CCFG-30 through CCFG-33 are closed.
3. Await explicit command-only amendment authority for the same
   `ccfg-25-planning-ownership-transfer` Slice 2. Preserve its verified candidate
   diff uncommitted and do not change the validator or truthful skill dependencies.
