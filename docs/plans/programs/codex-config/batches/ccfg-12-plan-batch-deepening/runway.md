...
| 3. Align docs, metadata, and closeout evidence | `c28bb08` | success; aligned user-facing docs, feature metadata, changelog, and closeout evidence for the deepened `plan-batch` command contract | `python -m pytest tests/test_codex_features_manifest.py -q`; `python -m pytest tests/test_batch_runway_create_spec_contract.py -q`; `python scripts/planning_state.py current --root docs/plans`; `python scripts/planning_state.py validate --root docs/plans`; `./install.sh --dry-run`; `git diff --check`; hard-coding check no matches; reviewer `runway_reviewer` clean |
