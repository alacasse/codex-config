...
and returns clean/correct/blocked. Only an exact evidence-backed clean result may
then authorize Architecture Program Runway to mutate the queue.
...
cannot enter the queue through an evidence-free clean review, while ordinary
...
- Approval gate for both slices: the user's later explicit `work-batch` while
  this exact master-only runway is the sole queue entry, master identity still
  matches, the prior slice is green where applicable, and the independent
  reviewer has accepted the exact preceding basis.
...
3. behavioral evidence that applicable bad plans correct or block,
   evidence-free clean review cannot queue, and the valid small plan still
   queues;
...
8. exact closeout evidence consumed by same-batch reconciliation.
...
- an evidence-free clean review can still authorize queue mutation;
