...
| 4. Non-goals and extraction stop gates | `860d123` | success; hardened CCFG-1 closeout gates, unresolved extraction decisions, and no-extraction/no-scaffold stop conditions across the contract, ledger, current state, dispatch, and runway | `python scripts/planning_state.py current --root docs/plans`; `python scripts/planning_state.py validate --root docs/plans`; `git diff --check`; reviewer `runway_reviewer` clean |
