...
| 3. Clarify program/runway handoff | Closed | this commit | `consumer_projection_routing.py` 14 passed; `current`; `validate`; `git diff --check` | Clean review | Specialized discovery role boundaries remain. | Architecture Program Runway owns program-level ledger/queue/closeout reconciliation; Batch Runway owns concrete execution ledgers, validation/review routing, commits, and completed-slice archives. |
