...
| 2. Route Architecture Program projection reports | Closed | this commit | `tests/test_planning_state_consumer_projection_routing.py` passed 6; architecture runner protocol tests passed 2; manifest tests passed 6; `current`/`validate` passed with existing redirect warnings only; `git diff --check` passed | `runway_reviewer` clean after staged-scope reconciliation | Consumer routing test for Legacy Removal passes | Architecture Program Runway now routes supported history/reporting questions through policy-compatible projection reports while preserving program ledger, dispatch, grouping, sequencing, and closeout ownership. |
| 3. Route Legacy Removal projection reports | Closed | this commit | `tests/test_planning_state_consumer_projection_routing.py` passed 9; manifest tests passed 6; `current`/`validate` passed with existing redirect warnings only; `git diff --check` passed | `runway_reviewer` clean | Manifest, changelog, diagnostics, and final hard-coding checks pass | Legacy Removal now routes supported history/reporting questions through policy-compatible projection reports as planning-state context only, preserving evidence-based legacy decisions. |
| 4. Align metadata and close consumer obligations | Closed | this commit | Final validation passed: consumer routing, manifest, and architecture protocol tests passed 17; `current`/`validate` passed with existing redirect warnings only; hard-coding scan found only one pre-existing neutral phrase; `git diff --check` passed | `runway_reviewer` clean after stale-ledger wording fix | None | PST-16 and PST-17 are closed; active and queued planning-state-tooling batch state is cleared without selecting the next batch. |
...
orchestration_anomalies:
  - slice: 4
    severity: low
    category: unexpected_head_movement
    observed: "After interruption, HEAD had advanced from `500cdd9` to include `bb42350` planning-doc updates and `58a5285` README changes."
    impact: "No worktree conflict; README change stayed out of scope, and Slice 4 resumed from the clean current baseline."
    action_taken: "Inspected the intervening commits, preserved them, and re-delegated Slice 4 from current HEAD."
    follow_up: "None."
...
| 2. Route Architecture Program projection reports | this commit | success; Architecture Program Runway now names projection-reporting guidance and `report-projection` for supported program history/reporting questions without letting projection reports select batches, replace ledgers or dispatch packets, or close findings | Validation: `python -m pytest tests/test_planning_state_consumer_projection_routing.py -q`, `python -m pytest tests/test_architecture_program_runner_protocol.py -q`, `python -m pytest tests/test_codex_features_manifest.py -q`, `python scripts/planning_state.py current --root docs/plans`, `python scripts/planning_state.py validate --root docs/plans`, `git diff --check`; review: clean `runway_reviewer` result after staged-scope reconciliation. |
| 3. Route Legacy Removal projection reports | this commit | success; Legacy Removal now names projection-reporting guidance and `report-projection` for supported history/reporting questions while treating projection output as context only, never legacy-classification evidence | Validation: `python -m pytest tests/test_planning_state_consumer_projection_routing.py -q`, `python -m pytest tests/test_codex_features_manifest.py -q`, `python scripts/planning_state.py current --root docs/plans`, `python scripts/planning_state.py validate --root docs/plans`, `git diff --check`; review: clean `runway_reviewer` result against the Slice 3 diff. |
| 4. Align metadata and close consumer obligations | this commit | success; closeout and completed-slice pointers summarize the consumer routing, manifest, changelog, and regression surfaces; PST-16 and PST-17 are closed with final validation and clean review evidence | Validation: `python -m pytest tests/test_planning_state_consumer_projection_routing.py tests/test_codex_features_manifest.py tests/test_architecture_program_runner_protocol.py -q`, `python scripts/planning_state.py current --root docs/plans`, `python scripts/planning_state.py validate --root docs/plans`, hard-coding `rg` scan, `git diff --check`; review: clean `runway_reviewer` result after stale-ledger wording fix. |
