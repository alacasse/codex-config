# Planning Root

`docs/plans/` is the active **Planning Root** for architecture-program runner
planning.

Active planning:

- `codex-config-architecture-program-runner-findings.md`: active Program
  Ledger.
- `phase-runner-repo-split-issue-12-plan.md`: current issue #12 decision note.
- `generic-phase-runner-product-idea.md`: product idea feeding the generic
  workflow contract.

Temporary compatibility exception:

- `plans/codex-config-phase-runner-extraction-prep-runway.md` and
  `plans/dispatch/phase-runner-extraction-prep-dispatch.md` remain under
  `plans/` until that executing batch closes.

Completed or superseded plans and reports live under `docs/plans/archive/`.
