...
class CrossCheckoutContextTests(unittest.TestCase):
    ...
    def test_live_lease_preflight_blocks_revision_and_identity_mismatches(
        ...
        self._move_head(self.implementation_root, "implementation-preflight")
        implementation_result = self._preflight()
        self.assertEqual(implementation_result.status, "blocked")
        self.assertIn(
            "implementation repository HEAD moved",
            implementation_result.reason,
        )

        self.tearDown()
        self.setUp()
        identity_mismatch = self._payload()
        execution = self._execution_payload(identity_mismatch)
        execution["toolchain_source_root"] = str(self.implementation_root)
        execution["toolchain_commit"] = self.implementation_commit

        identity_result = self._preflight(identity_mismatch)

        self.assertEqual(identity_result.status, "blocked")
        self.assertIn("generation_role 'stable' requires", identity_result.reason)

    def test_live_lease_preflight_context_expires_after_repository_movement(
        self,
    ) -> None:
        result = self._preflight()
        self.assertEqual(result.status, "ready")
        ...
        self._move_head(self.implementation_root, "after-preflight")
