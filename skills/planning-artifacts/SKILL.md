...
description: Define project-agnostic placement and naming conventions for durable planning docs, program ledgers, dispatch packets, Batch Runway specs, batch-local runtime state, runner receipts, generated outputs, active-state files, and archives. Use when creating, reorganizing, migrating, or interpreting planning roots and workflow artifacts.
---
...
the actual root path. This skill defines placement, naming, file shape, batch
directory conventions, archives, optional run artifact roots, and output roots.
...
this skill does not replace its pickup diagnostic, does not create a competing
pickup algorithm, and grants no queue, dispatch, runway, or lifecycle mutation
authority.
...
- `planning_root`: durable human-readable planning documents, program ledgers,
  and batch directories.
- `batch_runtime_policy`: normally `batch-local`; small machine state whose
  identity and lifecycle are one batch lives under that batch directory.
- `run_artifact_root`: optional runner-owned JSON state, receipts, manifests,
  telemetry, or bulky operational output whose identity is one runner run or an
  explicitly external backend. It is not a mandatory second root for one
  batch's canonical state.
...
The user-selected planning root is enough to locate a program ledger, its batch
directories, and small state owned by one batch. Do not require a second root
for that state. A project may still declare a separate run-artifact root for
runner-global, bulky, or external operational artifacts.
...
A missing optional `run_artifact_root` blocks only an operation that truly needs
runner-global, bulky, or external artifacts. It does not block batch-local state
when the accepted batch directory and state policy are known.
...
- `planning_root` contains human-authored or human-readable coordination docs and
  the directories that define program and batch identity.
- a batch's `.runtime/` contains small operational state whose identity and
  lifecycle are exactly that batch.
- `run_artifact_root`, when configured, contains operational runner evidence and
  machine state whose identity is one runner run, or an explicitly configured
  external backend.
...
put raw runner-global JSON, bulky receipts, telemetry, generated indexes, or
large tool output inside the active planning tree unless a project explicitly
declares that exception.

## Product And Dogfood Boundary

Storage layout follows the user-facing product, not the installer or dogfood
harness used by the current development repository.

Unless the user explicitly promotes one into the product, these values do not
shape generic product APIs, schemas, persistent state, storage layout, or
terminology:

- `CODEX_HOME` or `.codex` layout;
- symlink or feature-manifest installation;
- stable/candidate repositories, branches, or generations;
- cross-checkout bridge paths, leases, or receipts;
- developer-specific absolute paths.

A dogfood adapter may locate and launch product code using those values. Product
code and durable product contracts must remain usable without the adapter.
...
- planning root, batch runtime policy, optional run artifact root, and output root
...
- run artifact location for the program, or `None`
- batch runtime policy when it differs from the root default
- program archive location
- next safe action
- blockers, stop conditions, and explicit user-owned worktree constraints
...
Co-locate the selected dispatch packet, concrete Batch Runway spec, small
batch-owned runtime state, closeout, and completed-slice archive for one batch:
...
global `dispatch/`, `runways/`, state, and `closeout/` directories unless an
existing project convention or explicit external backend requires that
compatibility shape.
...
history. The project chooses whether `.runtime/` is `ignored-local`, `external`,
or `generated-only`.
...
Validate path components before constructing batch-local paths. Reject parent
traversal, absolute path injection, ambiguous separators, and collisions under
the target platform's normal filesystem rules.

Layout safety does not imply protection against a hostile same-user process
replacing filesystem namespace entries during a system call. Such a claim
requires an explicit threat model and a feasible cross-platform primitive.
...
- `runner-artifact`: runner-global JSON state, receipts, manifests, telemetry, or
  inventories.
- `batch-runtime`: small state owned by one batch.
...
1. Declare the new `planning_root`, `batch_runtime_policy`, optional
   `run_artifact_root`, and `output_root` in project instructions or local
   overlay before moving files.
...
6. Move active dispatch/runway/closeout sets and their small batch-owned state
   into batch directories, preserving their lineage.
7. Move runner-global JSON, bulky receipts, manifests, telemetry, and input
   inventories to `run_artifact_root` when one is configured; keep only links or
   summaries in planning docs.
...
- batch-local runtime state, when present, is beneath the owning batch directory
  or an explicitly declared external backend.
- old active paths are either absent, ignored by project rules, or contain
  redirect notes.
- runner-global JSON and generated outputs are outside active planning docs unless
  a project explicitly declares an exception.
...
- Keep small batch-owned runtime state under the batch directory; keep
  runner-global, bulky, or external operational artifacts in the optional run
  artifact root.
...
- Keep `dispatch.md`, `runway.md`, `.runtime/`, `closeout.md`, and
  completed-slice archives co-located.
