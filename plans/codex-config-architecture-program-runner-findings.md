...
| APR-13. Structured failure paths do not refresh manifests consistently | Open | Error path writes state and run telemetry; success path writes run/batch manifests | Include in the runner boundary batch if a small manifest-writer seam makes it cheap; otherwise keep as a focused follow-up | The runner writes telemetry on `RunnerError`, but `write_artifact_manifests` is only called on the successful phase path. |
