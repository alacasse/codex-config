...
| APR-11. `architecture_program_runner.py` concentrates too many responsibilities | Closed | Runner boundary slices 1-4; `scripts/architecture_program_runner_state.py`, `scripts/architecture_program_runner_validation.py`, `scripts/architecture_program_runner_command.py`, `scripts/architecture_program_runner_artifacts.py` | None | The CLI entrypoint now delegates path/state, result validation, prompt/command, and artifact/telemetry ownership to focused modules while preserving compatibility re-exports. |
| APR-12. Tests mirror the large runner file as one broad suite | Closed | Runner boundary Slice 5; `tests/architecture_program_runner_test_support.py`, `tests/test_architecture_program_runner_*` owner modules | None | Broad runner coverage is split across owner-seam modules plus a thin CLI/integration suite; review restored dropped regression checks before closeout. |
